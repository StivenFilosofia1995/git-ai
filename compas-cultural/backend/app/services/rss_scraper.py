...
# ─── Medios independientes de Medellín / Colombia con RSS confirmado ──────────
#
# Cada entrada: {feed_url, nombre, municipio, categoria_principal}
# Se usan como "lugares" sintéticos para parse_rss_events().

MEDIOS_INDEPENDIENTES_FEEDS: list[dict] = [
    # ── Medios culturales Medellín ─────────────────────────────────────────────
    {
        "feed_url": "https://www.universocentro.com/feed",
        "nombre": "Universo Centro",
        "municipio": "medellin",
        "categoria_principal": "cultura",
    },
    {
        "feed_url": "https://hacemosmemoria.org/feed",
        "nombre": "Hacemos Memoria",
        "municipio": "medellin",
        "categoria_principal": "cultura",
    },
    {
    ...
    # ── Medios culturales Colombia (eventos en Medellín) ───────────────────────
    {
        "feed_url": "https://www.elmalpensante.com/feed",
        "nombre": "El Malpensante",
        "municipio": "medellin",
    ...
    # ── Teatros / espacios con RSS ─────────────────────────────────────────────
    {
        "feed_url": "https://www.teatrometropolitano.com/feed",
        "nombre": "Teatro Metropolitano de Medellín",
        "municipio": "medellin",
        "categoria_principal": "teatro",
    },
    {
        "feed_url": "https://teatropablotobon.com/feed",
        "nombre": "Teatro Pablo Tobón Uribe",
        "municipio": "medellin",
        "categoria_principal": "teatro",
    },
    # ── Festivales y programación cultural ────────────────────────────────────
    {
        "feed_url": "https://festivaldepoesiademedellin.org/feed",
        "nombre": "Festival Internacional de Poesía de Medellín",
        "municipio": "medellin",
        "categoria_principal": "literatura",
    },
    {
        "feed_url": "https://platohedro.org/feed",
        "nombre": "Platohedro",
        "municipio": "medellin",
        "categoria_principal": "arte_contemporaneo",
    },
    # ── Agenda / guías de ocio ─────────────────────────────────────────────────
    {
        "feed_url": "https://www.culturama.co/feed",
        "nombre": "Culturama Colombia",
        "municipio": "medellin",
        "categoria_principal": "cultura",
    },
    {
        "feed_url": "https://www.laciudaddelosdioses.co/feed",
        "nombre": "La Ciudad de los Dioses",
        "municipio": "medellin",
...
async def scrape_medios_independientes_rss() -> dict:
    """
    Recorre todos los feeds de MEDIOS_INDEPENDIENTES_FEEDS, parsea eventos
    y los inserta en la BD. Deduplica por slug.

    Returns: {nuevos, duplicados, errores, feeds_ok, feeds_fallidos}
    """
    import unicodedata as _ud

    def _slug_date(slug: str, fecha: str) -> str:
        fecha_part = fecha[:10] if len(fecha) >= 10 else fecha
        return f"{slug}-{fecha_part}"

    print("\n📰 RSS Medios Independientes — iniciando...")

    from app.database import supabase as _sb

    nuevos = duplicados = errores = feeds_ok = feeds_fallidos = 0

    for medio in MEDIOS_INDEPENDIENTES_FEEDS:
        feed_url = medio["feed_url"]
        lugar_fake = {
            "id": None,
            "nombre": medio["nombre"],
            "municipio": medio.get("municipio", "medellin"),
            "categoria_principal": medio.get("categoria_principal", "otro"),
            "barrio": None,
        }

        try:
            events = await parse_rss_events(feed_url, lugar_fake)
        except Exception as e:
            print(f"  [RSS Medios] Error en {feed_url}: {e}")
            feeds_fallidos += 1
            continue
        ...
        for ev in events:
            # Build slug with date suffix
            slug_base = ev.get("slug", "")
            fecha = ev.get("fecha_inicio", "")
            slug = _slug_date(slug_base, fecha)
            ev["slug"] = slug
            ev["fuente"] = "rss_medio_independiente"
            ...
            try:
                ...
                if "duplicate" in err_str.lower() or "unique" in err_str.lower():
                    ...
                    print(f"  [RSS Medios] Insert error '{ev.get('titulo', '?')[:40]}': {e}")
                    errores += 1

    print(
        f"  [RSS Medios] Listo — "
        f"nuevos={nuevos} duplicados={duplicados} errores={errores} "
        f"feeds_ok={feeds_ok} feeds_fallidos={feeds_fallidos}"
    )
    return {
        "nuevos": nuevos,
